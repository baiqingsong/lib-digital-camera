...
import com.dawn.digital_camera.ptp.PacketUtil;
import com.dawn.digital_camera.ptp.PtpCamera.IO;
import com.dawn.digital_camera.ptp.PtpConstants;
import com.dawn.digital_camera.ptp.PtpConstants.Event;
    ...
    protected void decodeData(ByteBuffer b, int length) {
        while (b.position() < length) {
            ...
            if (AppConfig.LOG) {
                ...
                Log.i(TAG, "event event " + event);
                Log.i(TAG, "event typeStr " + PtpConstants.eventToString(event));
            }
            switch (event) {
                //对象添加通知
                case Event.CustomEOS_ADD:{
                    ...
                    Log.i("222222", "decodeData: "+objectHandle);
                ...
                case Event.EosObjectAdded: {
                    ...
                    int objectFormat = b.getShort();
                    skip(b, eventLength - 18);
                    Log.i("EosEventCheck", "EosObjectAdded: objectHandle=" + objectHandle + ", format=" + objectFormat);
                    camera.onEventDirItemCreated(objectHandle, storageId, objectFormat, "TODO");
                    break;
                }
