...
import com.dawn.digital_camera.ptp.PtpAction;
...
public interface Camera {
    ...
    /**
     * 优雅关闭相机连接，等待 WorkerThread 处理完当前任务后停止
     */
    void shutdown();

    /**
     * 强制关闭相机连接，立即停止 WorkerThread 和 USB 连接
     */
    void shutdownHard();
     ...
     * 设备忙时的处理方法
     */
    void onDeviceBusy(PtpAction action, boolean requeue);

    /**
     * PTP 错误处理
     */
    void onPtpError(String message);

//    /**
//     * 延迟添加命令到队列
//     */
//    void enqueue(Command cmd, int delay);
     ...
     * 处理 EOS 目录项目创建事件
     */
    void onEventDirItemCreated(int objectHandle, int storageId, int objectFormat, String todo);

    /**
     * 处理对象添加事件
     */
    void onEventObjectAdded(int objectHandle, int format);
     ...
     * 处理相机捕获状态事件
     */
    void onEventCameraCapture(boolean started);
}
